...
int main(int argc, char *argv[]){
	...
	if (record_e == 1){
		...
		uint32_t  nthumbnail = labs(xthumbnail) * labs(ythumbnail);
		...
		int32_t addre = ftell(pfile);
		fseek(pfile, addre + (xthumbnail) * (ythumbnail-1)*3, SEEK_SET);
		int32_t now = ftell(pfile);
		while (now >= addre){
			for (int32_t i = 0 ; i < xthumbnail; i++){
				fread(&cur, 1, sizeof(S_pixel), pfile);
				transfer(&cur);
				fwrite(&cur, 1, sizeof(S_pixel), pfile2);
			}
			now -= xthumbnail  * 3;
			fseek(pfile, now, SEEK_SET);
		}
	...
	//write
		...
		if (abs(header.width) > 255 || abs(header.height) >255){
		...
		//開始把bmp檔案寫入pfile
		int32_t addre = ftell(pfile2);
		fseek(pfile2, (xthumbnail) * (-1)*3, SEEK_END);
		int32_t now = ftell(pfile2);
		while (now >= addre){
			for (int32_t i = 0 ; i < xthumbnail; i++){
