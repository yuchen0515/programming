...
/*
   FILE *pfile = NULL;
   char *pfile_name = "message_output.txt";

   if ((pfile = fopen("message_output.txt", "w")) == NULL){
   perror("Error: ");
   return 0;
   }

   fprintf(pfile, "message123\n");
   fprintf(pfile, "message123\n");
   fprintf(pfile, "message123\n");
   fprintf(pfile, "message123\n");
   fprintf(pfile, "message123\n");
   fprintf(pfile, "message123\n");
   receive_message(pfile_name);
 */
...
void list_rank(int32_t score[], int32_t set_points, int32_t player_number){
	...
	for (int32_t i = 0 ; i < player_number; i++){
		...
		if (score[win_point[i].index] >= set_points)
			printf("(感染者)\n");
		else puts("");
	}
...
/*
   ...
   FILE *pfile = NULL;
   if ((pfile = fopen("message_output.txt", "r")) == NULL){
   perror("Error: ");
   return;
   }

   long address = ftell(pfile);
   ...
   fscanf(pfile, "%c", &temp);
   int32_t record = ftell(pfile);
   fgets(message, 9999, pfile);
   printf("%s", message);
   if (count > 5) break;

   fseek(pfile, record-1, SEEK_SET);
...
void init_all_val(){
	memset(card, 0, 105);
	memset(table_last_index, 0, 4);
	memset(row_table_point, 0, 4);
	memset(agent_level , 1, 10);
	player = 2;
	start_game_choose = 0;
	game_setting = 0;
	set_points = 172;
