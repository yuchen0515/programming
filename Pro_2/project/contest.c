        ...
        for( int32_t j = i + 1 ; j < NUMBER ; j++ )
        {
            ...
            for( int32_t k = 0 ; k < 1001 ; k++ )
            {
                ...
                for( int32_t l = 0 ; l < 10 ; l++ )
                {
                	...
                	int32_t value[2];
					value[0] = player[i].pick( table , score , last1, card0); 
            		value[1] = player[j].pick( table , score , last0, card1);
					...
					score[0] = player[i].score;
					score[1] = player[j].score;
                }
                //end play
                if(detect == 0)
                {
